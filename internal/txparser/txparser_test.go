...
import (
	"context"
	svc "github.com/Dan9-1337/assesment-trustwallet/internal"
	txparser2 "github.com/Dan9-1337/assesment-trustwallet/internal/txparser"
	"github.com/Dan9-1337/assesment-trustwallet/pkg/ethclient"
	"testing"
)
...
func TestTxParserService(t *testing.T) {
	...
	service := txparser2.New(context.Background(), Endpoint, InitialBlock)
...
func makeSampleDataset(t *testing.T, ethclt *ethclient.Client, initialBlock, finalBlock int) map[string][]svc.Transaction {
	...
	for i := initialBlock; i <= finalBlock; i++ {
		...
		for _, tx := range block.Transactions {
			dataset[tx.To] = append(dataset[tx.To], txparser2.ParseTx(tx))
			dataset[tx.From] = append(dataset[tx.From], txparser2.ParseTx(tx))
		}
	}
	return dataset
}
